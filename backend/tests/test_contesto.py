...
# ===================================================================
# Test: celebrazione promozione nella direttiva
# ===================================================================


class TestCelebrazionePromozione:
    """Test che _genera_direttiva include il contesto di celebrazione
    quando promozione_appena_avvenuta è presente nello stato_orchestratore,
    e che il flag viene consumato (azzerato) dopo l'uso.
    """

    @pytest.mark.asyncio
    async def test_direttiva_con_promozione_contiene_celebrazione(self):
        """La direttiva post-promozione deve contenere il nome del nodo completato."""
        from unittest.mock import AsyncMock, MagicMock, patch

        from app.core.contesto import _genera_direttiva

        class FakeSessione:
            tipo = "studio"
            stato_orchestratore = {
                "attivita_corrente": "spiegazione",
                "promozione_appena_avvenuta": {
                    "nodo_id": "eq_1_grado",
        ...
        class FakeNodo:
            id = "eq_2_grado"
            nome = "Equazioni di secondo grado"
            definizioni_formali = None
            formule_proprieta = None
            errori_comuni = None
        ...
        # La direttiva deve contenere la celebrazione
        assert "PROMOZIONE APPENA AVVENUTA" in result
        assert "Equazioni di primo grado" in result
        assert "Equazioni di secondo grado" in result
        # E anche la direttiva di spiegazione normale
        assert "ATTIVITÀ: Spiegazione" in result
        # Il flag deve essere stato rimosso dallo stato_orchestratore
        assert "promozione_appena_avvenuta" not in sessione.stato_orchestratore

    @pytest.mark.asyncio
    async def test_direttiva_senza_promozione_non_contiene_celebrazione(self):
        """Senza promozione, la direttiva è normale."""
        from unittest.mock import AsyncMock, MagicMock, patch
        ...
        assert "PROMOZIONE APPENA AVVENUTA" not in result
        assert "ATTIVITÀ: Spiegazione" in result

    @pytest.mark.asyncio
    async def test_flag_promozione_consumato_dopo_primo_uso(self):
        """Il flag promozione_appena_avvenuta deve essere consumato:
        la seconda chiamata non deve più contenere la celebrazione.
        """
        from unittest.mock import AsyncMock, MagicMock, patch
        ...
        with (
            ...
            result1 = await _genera_direttiva(db, sessione, FakeUtente(), FakeNodo())
            assert "PROMOZIONE APPENA AVVENUTA" in result1

            # Seconda chiamata: celebrazione NON presente (flag consumato)
            result2 = await _genera_direttiva(db, sessione, FakeUtente(), FakeNodo())
            assert "PROMOZIONE APPENA AVVENUTA" not in result2
