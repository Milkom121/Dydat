...
| 2026-04-08 | Primo turno di studio: messaggio monolitico a tre battute (saluto+profilo, presentazione nodo, domanda warm-up) | Lo studente ha gia fatto 8 turni di onboarding, altri 2-3 turni prima del merito sarebbero frustranti. Un singolo messaggio streamato diviso in 3 blocchi da sostanza subito senza muri di testo | Multi-turno puro (3 messaggi separati), monolitico denso unico blocco |
| 2026-04-08 | Approccio parzialmente deterministico per primo turno: sistema inietta nome utente + titolo nodo, LLM genera il resto | Nome sbagliato o titolo nodo storpiato sono errori non perdonabili in un "primo turno caldo". Il resto e voce del tutor e va tenuto vivo, non irrigidito in template | Tutto LLM libero (rischio errori nome), template a slot rigido (uccide il tono) |
| 2026-04-08 | Uso parafrasato (non verbatim) del profilo utente nel primo turno — campi chi_e, motivo, stile_cognitivo | La citazione verbatim "hai detto X" suona come servizio clienti che rilegge il ticket. La parafrasi mostra comprensione invece di archivio | Citazione verbatim esplicita, uso solo implicito nel tono (troppo sottile, utente non percepisce) |
| 2026-04-08 | Helper `_preambolo_caldo` riutilizzabile tra direttive di "primo momento" | Evita duplicazione e garantisce coerenza di tono tra spiegazione nuovo nodo, ripresa sessione, futuro primo esercizio e ripasso. Ogni direttiva mantiene la sua logica specifica dopo il preambolo | Applicare solo a `direttiva_spiegazione` (incoerenze tra flussi), unificare tutto in super-direttiva (troppo accoppiato) |
| 2026-04-08 | Piano sessione strutturato rimandato — B33.5 si limita a citazione leggera del tempo + modulazione morbida | Un vero piano con budget per fase richiede ripensare il modello temporale di Dydat alla luce delle neuroscienze dell'apprendimento mobile (micro-cicli 3-7 min, richiamo attivo, ripetizione distanziata). Villa ha chiesto esplicitamente di non partire di pancia e studiare prima. Parcheggiato in `.claude/ideas.md` 2026-04-08 | Implementare subito il piano (rischio numeri inventati), solo citazione testuale senza enforcement (promessa al vento) |
| 2026-04-08 | Nodi presunti padroneggiati: primo turno differenziato con parametro `nodo_presunto` nella direttiva | Trattare un nodo presunto come nuovo e un insulto allo studente e spreca il suo tempo. Il placement test onboarding serve esattamente a evitare questo | Trattamento uguale (ignora il profilo), skip totale del nodo (placement e probabilistico, non certo) |
